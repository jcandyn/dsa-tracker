...
const Navbar = () => {
  ...
  return <header className="flex h-16 items-center justify-between border-b border-border bg-background px-6">
    ...
    <div className="flex items-center gap-2">
      <button type="button" onClick={() => updateSettings({ theme: theme === "dark" ? "light" : "dark" })} className="inline-flex h-10 w-10 items-center justify-center rounded-lg text-slate-600 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-white" title={`Switch to ${theme === "dark" ? "light" : "dark"} theme`}>{theme === "dark" ? <Sun size={20} /> : <Moon size={20} />}</button>
      <div className="relative"><button type="button" onClick={() => setShowNotifications((show) => !show)} className="relative inline-flex h-10 w-10 items-center justify-center rounded-lg text-slate-600 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-white" title="Notifications"><Bell size={20} />{reviewNotifications && reviewCount > 0 && <span className="absolute -right-2 -top-2 flex h-4 min-w-4 items-center justify-center rounded-full bg-sky-500 px-1 text-[10px] text-white">{reviewCount}</span>}</button>{showNotifications && <div className="absolute right-0 top-11 z-50 w-72 rounded-xl border border-slate-700 bg-slate-900 p-4 shadow-xl"><p className="font-medium text-white">Notifications</p><p className="mt-2 text-sm text-slate-400">{reviewNotifications ? reviewCount ? `${reviewCount} problem${reviewCount === 1 ? "" : "s"} ready for review.` : "You are all caught up." : "Review notifications are muted in Settings."}</p></div>}</div>
    </div>
  </header>;
};

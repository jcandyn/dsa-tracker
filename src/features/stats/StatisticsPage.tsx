...
const StatisticsPage = () => {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-xl border border-slate-800 bg-slate-900 p-6"><h2 className="font-semibold text-white">Study heatmap</h2><p className="mt-1 text-sm text-slate-400">Your last 35 days of activity.</p><div className="mt-5 grid w-fit grid-cols-7 gap-1.5">{heatmap.map((day) => <div key={day.key} title={`${day.label}: ${day.count} activities`} className={`h-4 w-4 rounded-sm ${day.count === 0 ? "bg-slate-800" : day.count === 1 ? "bg-emerald-800" : day.count === 2 ? "bg-emerald-600" : "bg-emerald-400"}`} />)}</div></section>

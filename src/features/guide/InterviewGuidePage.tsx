...
const InterviewGuidePage = () => (
  <div className="mx-auto max-w-5xl space-y-8">
    ...
    <section className="rounded-xl border border-slate-800 bg-slate-900 p-6"><h2 className="flex items-center gap-2 text-xl font-semibold text-white"><CheckCircle2 size={20} className="text-emerald-400" />Recommended progression</h2><p className="mt-3 text-sm text-slate-300">The <strong className="text-white">Problem type</strong> dropdown on the Problems page is arranged in this order. Within each topic, use <strong className="text-white">Sort → Difficulty: Easy first</strong>, then work up through Medium and Hard problems as you gain confidence.</p><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 text-sm"><p className="rounded-lg bg-slate-950 p-4 text-slate-300">Arrays, hashing, two pointers, and sliding window</p><p className="rounded-lg bg-slate-950 p-4 text-slate-300">Linked lists, stacks, queues, binary search, and intervals</p><p className="rounded-lg bg-slate-950 p-4 text-slate-300">Trees, heaps, graphs, recursion, and backtracking</p><p className="rounded-lg bg-slate-950 p-4 text-slate-300">Dynamic programming, greedy, and math patterns</p><p className="rounded-lg bg-slate-950 p-4 text-slate-300">Timed mixed practice and repeat mistakes until fluent</p><p className="rounded-lg bg-slate-950 p-4 text-slate-300">Mock interviews plus behavioral-story preparation</p></div></section>

    <section className="rounded-xl border border-violet-500/30 bg-violet-500/10 p-6"><h2 className="flex items-center gap-2 text-xl font-semibold text-white"><MessageCircle size={20} className="text-violet-300" />Interview habits that matter</h2><p className="mt-3 text-sm text-slate-300">Clarify assumptions, narrate your approach before coding, test edge cases, and discuss tradeoffs. Completing every problem is less important than confidently explaining the core patterns.</p><div className="mt-5 flex flex-wrap gap-3"><Link to="/problems" className="rounded-lg bg-sky-100 px-4 py-2 text-sm font-medium text-sky-700 hover:bg-sky-200 dark:bg-sky-500 dark:text-white dark:hover:bg-sky-600">Browse problems</Link><Link to="/review" className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 dark:border-slate-600 dark:bg-transparent dark:text-slate-200 dark:hover:bg-slate-800">Open review queue</Link></div></section>
  </div>
);
